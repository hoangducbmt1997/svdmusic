...
@section('main')
    <form action="{{route('admin.song.update',$song->id)}}" enctype="multipart/form-data" role="form" method="POST">
        @csrf @method('PUT')
      ...
      <div class="col-md-6">
        ...
        <div class="form-group">
          <label for="">Top 100</label>
          <select class="form-control" name="top100_id">
              <option value="">-- Select One --</option>
              @foreach ($top100s as $item)
              <?php $selected = $song->top100_id == $item->id ? 'selected': ''; ?>
                  <option {{$selected}} value="{{ $item->id }}">{{ $item->name_top100 }}</option>
              @endforeach
          </select>
          @error('top100_id')
              <small id="helpId" class="text-muted">{{ $message }}</small>
